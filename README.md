...
Interactively the installer asks for install scope first (`user` global vs `project` folder), then offers only detected hosts compatible with that scope, then a checklist of modules compatible with the selected scope and host. (`--scope local` remains a Claude-only flag, not a menu option.)
