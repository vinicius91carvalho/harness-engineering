...
`config/settings.json` is written at the repository root (not under
`config/home/`): a committed, shareable subset of `~/.claude/settings.json`
(e.g. `model`, `worktree.baseRef`, notification preferences). Merged in by the
installer's "shared config" prompt.

It is absent until there's something to back up, and regenerated by
`/harness:update-project`, not hand-edited.
