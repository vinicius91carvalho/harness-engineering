...
| Cursor (`agent` / agent/cursor) | Plugin: `~/.cursor/plugins/local/<name>/`; CLI skills: `~/.cursor/skills/<skill>/` (copied from the plugin; real dirs, not symlinks) | Plugin: `$PROJECT/.cursor/plugins/local/<name>/`; CLI skills: `$PROJECT/.cursor/skills/<skill>/` (copied) |
