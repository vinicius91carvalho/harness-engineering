...
- `hallmark`, `no-mistakes`, and `treehouse` are not Claude marketplace plugins.
  The catalog records their acquisition policy and allows `user` and `project`:
  ...
  - `installer` / `installerWindows` — upstream shell or PowerShell one-liner
    (no-mistakes, treehouse). Under project scope, `no-mistakes init` also runs
    in `--project-dir`; under user scope the installer only installs the binary
    and reminds you to run `no-mistakes init` per repository.
- Playwright uses Microsoft's upstream host-neutral MCP server and supports
  ...
  scope; OpenCode/Cursor write the MCP config under the resolved base).
