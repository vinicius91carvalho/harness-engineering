...
harness local plugin under `.cursor/plugins/local/harness/` and copies each skill
into `.cursor/skills/` as real `harness-*` directories so both the IDE and the Agent
CLI discover `/harness-supervisor` and related skills. A piped PowerShell run stages the repository before it
