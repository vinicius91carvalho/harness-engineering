...
(Worktree Runtime Lifecycle). Base it on the spec's tech stack.

**Source of truth:** copy and adapt
`skills/generator/templates/init.sh` from the harness plugin (relative to the
installed plugin root; in this repo that path is
`skills/generator/templates/init.sh`). Keep the subcommand dispatch and
`.harness/app.pid` / health invariants; fill only the stack-specific `cmd_start`
body (deps install, daemonize, bind to `PORT` / `FRONTEND_PORT` / `BACKEND_PORT`).

It MUST expose these subcommands (no-arg defaults to `start`):
...
Behavioral contract (must match the template):
...
Do not paste a second full bash skeleton here - the template file is authoritative.
