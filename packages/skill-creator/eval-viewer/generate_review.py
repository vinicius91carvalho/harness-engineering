#!/usr/bin/env python3
import argparse, base64, json, mimetypes, os, re, signal, subprocess, sys, time, webbrowser
...
def load_previous_iteration(workspace):
    result = {}
    feedback_map = {}
...
def main():
    ...
    if not workspace.is_dir():
        print(f"Error: {workspace} is not a directory", file=sys.stderr); sys.exit(1)
    runs = find_runs(workspace)
    if not runs:
        print(f"No runs found in {workspace}", file=sys.stderr); sys.exit(1)
